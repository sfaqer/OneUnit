...
То, до чего прогон не дошёл, не пропадает из отчётов: наборы и тесты за точкой остановки попадают в них пропущенными с причиной «Прогон прекращён на первом падении». Счётчики поэтому показывают весь набор целиком, а не только исполненную часть.
