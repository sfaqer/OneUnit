...
Совет можно собрать и строителем autumn 5 (`ПолучитьСтроительМенеджераПараметров()`), и устаревшим `ЗначенияДеталек`. Служебные детальки изолированного прогона (файл набора, фильтр теста, таймаут) OneUnit добавляет к совету отдельным провайдером с наивысшим приоритетом: ни совет, ни секция `OneUnit` в настройках проекта их не перекрывают, остальные детальки доходят до изолированной поделки как обычно.
...
Опции:
  ...
  -M, --testMethodsNameExclude  Тестовые методы исключаемые из тестового прогона
      ...
      --failFast                Прекратить прогон после первого упавшего теста.
                                Набор, в котором тест упал, сворачивается штатно: ПослеКаждого
                                и ПослеВсех отрабатывают. То, до чего прогон не дошёл, попадает
                                в отчёты пропущенным
```
...
### 2.1.3 Прекращение прогона на первом падении

Опция `--failFast` останавливает прогон, как только что-то упало:

```sh
oneunit execute -d tests --failFast
```

Это нужно там, где важен только ответ «зелено или нет», а не полная картина: проверка отдельного коммита, мутационное тестирование, локальный цикл правка-прогон.

Прогон именно прекращается, а не обрывается. Набор, в котором упал тест, сворачивается штатно: `ПослеКаждого` упавшего теста и `ПослеВсех` набора отрабатывают, и открытые ими файлы, соединения и временные каталоги закрываются.

Останавливает прогон любое падение, откуда бы оно ни пришло: упавший тест обычного набора, изолированный тест, тесты изолированного набора, а также набор, красный без единого упавшего теста — со сломанным конструктором или `ПередВсеми`.

Пропущенные и прерванные тесты прогон не останавливают: они ничего не говорят о работоспособности кода. Таймаут — говорит, поэтому останавливает.

То, до чего прогон не дошёл, не пропадает из отчётов: наборы и тесты за точкой остановки попадают в них пропущенными с причиной «Прогон прекращён на первом падении». Набор, в котором прогон встал, поэтому виден целиком, а не только исполненной частью.

Набор целиком за точкой остановки своих тестов не разворачивает — как и набор, выключенный условием: он пропущен и в счёте тестов не участвует. То же с параметризованным тестом, до которого прогон не дошёл, — пропущен он сам, а не каждый его случай.

Той же настройкой управляет деталька `OneUnit.ПрерыватьПриПервомПадении` — она пригодится тем, кто поднимает прогон программно, минуя командную строку. Опция командной строки просто записывает её значение, а прогон передаёт настройку своим изолированным дочерним прогонам.
...
Параметр `fail_fast` инструмента `run_tests` прекращает прогон на первом падении — см. [раздел 2.1.3](#213-прекращение-прогона-на-первом-падении).
