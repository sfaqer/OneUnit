...
Процедура ИсполнениеТестКонец(Тест, Результат) Экспорт
	...
	Если Результат.ПеренесенноеВремяВыполнения() <> Неопределено Тогда
		// Тест выполнялся в изолированной среде, сюда доставлен только его итог:
		// начало отсчитывается от завершения на измеренное там время
		ПеренестиВремяВыполнения(СтатистикаТеста(Тест), Результат.ПеренесенноеВремяВыполнения());
	КонецЕсли;
...
Процедура ПеренестиВремяВыполнения(Статистика, Знач Миллисекунды)

	ВремяНачала = Статистика.ПолучитьИлиУмолчание("ВремяЗавершения", 0) - Миллисекунды;

	Статистика.Вставить("ВремяНачала", ВремяНачала);
	Статистика.Вставить("ДатаНачала", МестноеВремя('00010101' + (ВремяНачала / 1000)));
