...
// Последней строки-продолжения здесь нет намеренно: у --mode на неё ложится
// "(по умолчанию tree)", а у булевой опции умолчание не печатается, и осталась бы
// пустая строка из пробелов
&Опция(Имя = "failFast", Описание = "Прекратить прогон после первого упавшего теста.
...
Процедура Тестировать() Экспорт
	...
	Детальки.Вставить("ПрерыватьПриПервомПадении",   ПрерыватьПриПервомПадении);
	...
	_ЛогДляКомандыМенеджера.Дописать();
